...
import { InputBlockType, publicProbotSchema } from '@typebot.io/schemas'
...
export const generateUploadUrl = publicProcedure
  ...
  .mutation(async ({ input: { filePathProps, fileType } }) => {
    ...
    if ('probotId' in filePathProps) {
      const publicprobot = await prisma.publicProbot.findFirst({
        where: {
          probotId: filePathProps.probotId,
        },
        select: {
          groups: true,
          probot: {
      ...
      const workspaceId = publicprobot?.probot.workspaceId
      ...
      const filePath = `public/workspaces/${workspaceId}/probots/${filePathProps.probotId}/results/${filePathProps.resultId}/${filePathProps.fileName}`

      const fileUploadBlock = publicProbotSchema._def.schema.shape.groups
        .parse(publicprobot.groups)
    ...
    const probotId = session.state.probotsQueue[0].probot.id
    ...
    const resultId = session.state.probotsQueue[0].resultId

    const filePath = `public/workspaces/${workspaceId}/probots/${probotId}/results/${resultId}/${filePathProps.fileName}`
